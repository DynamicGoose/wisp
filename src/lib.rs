...
pub struct RenderState<'w> {
    ...
    pub light_buffers: Vec<wgpu::Buffer>,
...
impl<'w> RenderState<'w> {
    pub async fn new(window: &'w Window) -> Self {
        ...
        let lights: Vec<LightUniform> = vec![];
        // We'll want to update our lights's positions, so we use COPY_DST
        let light_buffers = lights
            .iter()
            .map(|light| {
        ...
        let light_bind_groups = light_buffers
            .iter()
            .map(|light_buffer| {
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
            ...
            });
        {
            ...
            self.models
                .iter()
                .map(|model| {
                    for i in 0..self.light_bind_groups.len() {
                        render_pass.draw_model_instanced(
                            model,
                            0..1_u32,
                            &self.camera_bind_group,
                            &self.light_bind_groups[i],
                        );
                    }
                })
                .count();
